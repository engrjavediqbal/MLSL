...
from util1.io import BatchFetcherGroup
from util1.sampler import FixedSampler, RandomSampler
from util1.util import get_interp_method, load_image_with_cache
...
def parse_split_file(dataset, split, num_sel_source = 500, num_source = 500, seed_int = 0, dataset_tgt='', split_tar='', data_root='', data_root_tgt='',gpus='0'):
    ...
    if not dataset_tgt == '':
        split_filename_tgt = 'issegm1/data_list/{}/{}_training_gpu{}.lst'.format(dataset_tgt, split_tar,gpus)
...
class FileIter(DataIter):
    ...
    def __init__(self,
                 ...
                 data_root_tgt = '',
                 data_name = 'data',
                 label1_name = 'softmax_label',
		 label2_name = 'sigmoid_label',
                 sampler = 'fixed',
                 has_gt = True,
                 batch_images = 1,
                 meta = None,
	...
	self._label2_name = label2_name
        ...
        self._label = list({self._label1_name: np.zeros((1, crop_h * crop_w / self._label_stride**2), np.single)}.items())
	self._label2 = list({self._label2_name: np.zeros((1,19), np.single)}.items())

    def read(self, db_inds):
        return self._reader(db_inds)
    
    def _read_transformer(self, db_inds):
	#print 'I am here at the transformer'
        output_list = []
        output_shape = [0, 0]
	...
	#print 'db_inds', len(db_inds)
	    ...
	    output2_label = np.zeros((self._batch_images, 19), np.single)
            output.append([output_data, output_label, output2_label])            
	    #output.append([output_data, output_label])
	#print ('output batch:', type(output), len(output))
	    ...
	    #print 'db_inds', len(db_inds)
            ...
            ##########
	    #print ('I am here at label reading :', self._label_list[db_ind])
	    #print ('db_ind :', db_ind)
            label = np.array(load_image_with_cache(self._label_list[db_ind], self._cache))

	    labelIDs = self._label_list[db_ind].split('/')
	    if labelIDs[0]=='gta2city':
		namesLabelIDs = labelIDs[-1].split('_')
		label1 = np.array(load_image_with_cache('data/cityscapes/gtFine/train/'+namesLabelIDs[0]+'/'+labelIDs[-1][:-15]+'gtFine_labelIds.png', self._cache))
		#print 'data/cityscapes/gtFine/train/'+namesLabelIDs[0]+'/'+labelIDs[-1][:-15]+'gtFine_labelIds.png', label1
		nlabel1 = self._ignore_label * np.ones((nim_size, nim_size), np.uint8)
            # label = olabel[shift_h:shift_h + h, shift_w:shift_w + w]
            ##########
            if self._origin_list[db_ind] == 0:
                label = label_2_id_src[label]
		if labelIDs[0]=='gta2city':
		    label1 = label_2_id_src[label1]
		    nlabel1[d0:d0 + h, d0:d0 + w] = label1
		
            elif self._origin_list[db_ind] == 1:
                label = label_2_id_tgt[label]
		if labelIDs[0]=='gta2city':
		    label1 = label_2_id_tgt[label1]
		    nlabel1[d0:d0 + h, d0:d0 + w] = label1
			...
			if labelIDs[0]=='gta2city':
			    tlabel1 = nlabel1[sy[k] + dy[j] - int(crop_size / 2):sy[k] + dy[j] + remain_minus,sx[k] + dx[j] - int(crop_size / 2):sx[k] + dx[j] + remain_minus]
			...
			if labelIDs[0]=='gta2city':
			    tlabel1 = nlabel1[sy[k] + dy[j]:sy[k] + dy[j] + crop_size,sx[k] + dx[j]:sx[k] + dx[j] + crop_size]
                        assert tlabel.shape[0] == tlabel.shape[1] == crop_size
		    ...
		    if labelIDs[0]=='gta2city':
			rlabel1 = Image.fromarray(tlabel1).resize((target_crop_size,target_crop_size), Image.NEAREST)
			rlabel1 = np.array(rlabel1)[lsy : target_crop_size : self._label_stride, lsx : target_crop_size : self._label_stride]
                    # flipping
                    if do_flipping:
                        rim = rim[:, ::-1, :]
                        rlabel = rlabel[:, ::-1]
		    ...
		    uniqueLabels = np.unique(rlabel)
		    if labelIDs[0]=='gta2city':
			rlabel1 = rlabel.copy() # for pseudo weak labels, need to commented for orig week labels
		        uniqueLabels = np.unique(rlabel1)
		    else:
			rlabel1 = rlabel.copy()
		    #print 'states : ', i, np.unique(rlabel), np.unique(rlabel1)


		    flatenLabel = rlabel1.flatten()
		    #print('shape :', flatenLabel.shape[0], np.array(rlabel1).shape)

		    sDist = 0.0225*flatenLabel.shape[0]* np.array([0.3232,0.085,0.1693,0.0193,0.0075,0.0107,0.0018,0.0013,0.0765,0.0220,0.1364,0.0039,0.0023,0.0274,0.0169,0.0181,0.0143,0.0019,0.0031])

		    #uniqueLabels = np.unique(rlabel)
		    hh, cc = np.histogram(rlabel1, bins=range(21))

		    #print 'states : ', i, uniqueLabels, uniqueLabels[:-1]
		    ...
		    sigmLabels = np.zeros((1,19), np.single)
#		    sigmLabels[0,uniqueLabels] = 1

		    sigmLabels[0,idx4] = 1
		    #print ('weak labels :', sigmLabels, uniqueLabels)
...
#		    sigmLabels[0,uniqueLabels] = 1
                    ...
                    output[k*locs_per_crop+j][0][i,:] = rim.transpose(2,0,1)
                    output[k*locs_per_crop+j][1][i,:] = rlabel.flatten()
		    output[k*locs_per_crop+j][2][i,:] = sigmLabels.flatten()
	#print ('output batch:', type(output), len(output))
        return output
	...
	label1 = [(k, tuple([self.batch_size] + list(v.shape[1:]))) for k, v in self._label]
	label2 = [(k, tuple([self.batch_size] + list(v.shape[1:]))) for k, v in self._label2]
        #return [('softmax_label', label1), ('sigmoid_label', label2)]
    ...
    def next(self):
	    ...
	    #print 'I am here at fetcher' 
            #print 'Waited for {} seconds'.format(time.time() - tic)
        else:
            raise StopIteration
        
        return DataBatch(data=[array(data_batch[0])], label=[array(data_batch[1]), array(data_batch[2])])
