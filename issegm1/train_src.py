...
def _eval_func(label, pred):
	...
	#print 'gt label shape:', gt_label.shape
	...
	#print sum_metric, num_inst + (num_inst == 0)
...
def _get_module(args, margs, dargs, net=None):
    if net is None:
        ...
        if model_specs['net_type'] == 'rna':
            ...
            # load network
                ...
                sym = import_module('util1.symbol.resnet')
    ...
    netLayers = net.list_arguments()
   # print netLayers
    fixed_param_names = [netLayers[name] for name in range (len(net.list_arguments())-9)]
    mod = mx.mod.Module(net, context=contexts, label_names=['softmax_label', 'sigmoid_label'])
    #mod = mx.mod.Module(net, context=contexts, label_names=['softmax_label', 'sigmoid_label'], fixed_param_names=fixed_param_names)
    return mod
