...
def _val_impl(args, model_specs, logger):
    ...
    for i in xrange(x_num):
        ...
        if do_forward:
            im = transformer(rim)
            imh, imw = im.shape[:2]
	    print('Origianl shape', rim.shape, im.shape)
            # init
            if batch is None:
                if dargs.ident_size:
	    ...
	    interp_preds_np = np.zeros((19, imh, imw))
		    ...
		    input_label = 255 * np.ones((1, imh*imw/64), np.single)
		    batch.label[1] = mx.nd.array(input_label)
	    ...
	    patchSize = 512
	    nPatches = 25
	    pixelCount = np.ones((interp_preds_np.shape[1], interp_preds_np.shape[2]))

	    #print('Image shape', interp_preds_np.shape, pixelCount.shape, imh, imw)

	    #interp_preds_Own = np.zeros((19, imh, imw))


	    #net_preds = np.zeros((margs.classes, pred_h, pred_w), np.single)

	    startRow = np.random.randint(0, imh-patchSize, size=nPatches)
	    startCol = np.random.randint(0, imw-patchSize, size=nPatches)
	    ...
	    for patch in range(nPatches):
		    ...
		    for ix in xrange(test_steps):
		        for iy in xrange(test_steps):
		            ...
		            input_data = np.zeros((1, 3, patchSize, patchSize), np.single)
			    pixelCount_temp = np.zeros((pixelCount.shape[0], pixelCount.shape[1]))
			    ...
			    colIdx = startCol[patch]
		            ...
		            input_data[0, :, :, :] = nim[:, rowIdx:rowIdx + patchSize, colIdx:colIdx + patchSize]
			    pixelCount_temp[rowIdx:rowIdx + patchSize, colIdx:colIdx + patchSize] = 1
			    pixelCount += pixelCount_temp


		            batch.data[0] = mx.nd.array(input_data)
			    input_label = 255 * np.ones((1, patchSize * patchSize/64), np.single)
			    batch.label[1] = mx.nd.array(input_label)
			    #print('shape :', batch.data[0].shape, batch.label[1].shape)
		            mod.forward(batch, is_train=False)
		            this_call_preds = mod.get_outputs()[0].asnumpy()[0]
		    ...
		    interp_preds_temp = interp_preds_as(input_data.shape[2:], this_call_preds, pred_stride, patchSize, patchSize)
		    #print(rowIdx, colIdx)
		    interp_preds_np[:,rowIdx:rowIdx + patchSize, colIdx:colIdx + patchSize] += interp_preds_temp


	interp_preds_np = interp_preds_np/pixelCount
