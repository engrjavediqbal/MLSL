...
def rn_top_1(feat, fc_name, classes):
    #pool7 = pool(feat, 'pool7', pool_type='avg', global_pool=True)
    scores = fc(feat, fc_name, classes)
    return softmax_out(scores)
...
def fcrna_model_a1_1(classes, inv_resolution=8, bootstrapping=False):
    ...
    return fcn_top_1(feat, classifier1, 'linear{}'.format(classes), classifier2, 'linear{}'.format(classes), classes, bootstrapping)


def fcrna_model_a1_Features(classes, inv_resolution=8, bootstrapping=False):
    '''FCRNA Model A1_1'''
    feat = rna_feat_a1(inv_resolution, dropout=True)
