...
def _val_impl(args, model_specs, logger):
    ...
    for i in xrange(x_num):
	    ...
	    interp_preds_np = oneHot(interp_preds_np)
	    ...
	    for patch in range(nPatches):
		    ...
		    #print(rowIdx, colIdx)
		    interp_preds_temp = oneHot(interp_preds_temp)
...
def oneHot(batch):
    k, n, m = batch.shape
    agm = np.argmax(batch, axis=0)
    Lhot = np.zeros((k, n*m))
    Lhot[agm.flatten(),np.arange(n*m)] = 1
    Lhot = Lhot.reshape(k, n, m)
    return Lhot
